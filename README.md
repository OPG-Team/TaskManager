...
Чувствительные данные храняться в `.env` и не выгружались в общий доступ на **GitHub**, а приводился пример файла для удобной последующей настройки.
...
- **POST** `/register` - регистрация нового аккаунта, принимает **json**:
    - email: string;
    - password: string.
- **POST** `/login` - авторизация и вутентификация пользователя, возврат JWT, принимает **json**:
    - email: string;
    - password: string.
