...
Также нужно настроить `.env`, а если быть точнее создать `.env` и скопировать данные из `example.env` и поменять на нужные вам настройки. Если оставить стандартные настройки для **SMTP_HOST** и **SMTP_EMAIL**, то сервис для отправки данных по почте будет отключен.
